...
def parse_parameter():
    ...
    parser.add_argument("-external", "--external_data",
                        required = True,
                        help = "file name of test data.")
    ...
    parser.add_argument("-mdl", "--model_str",
                        required = False,
                        help = "file name of best model, will traing model with the best model if provided. (e.g., best_model.pt)")
...
def fit_predict(ARGS, data_dict, hypergraph_dict, subgraph_dict):
    ...
    if args.model_str is not None:
        print(f'using pre-trained model={args.model_str}')
        # 1. sanity check on file path
        if pathlib.Path(args.model_str).is_file():
            model.load_state_dict(torch.load(args.model_str))
...
def train_cv(ARGS):
    ...
    for i in range(0,args.k_int):
        ...
        external_fname = args.external_data
        data_dict['train'] = pd.concat([pd.read_pickle(train_fname), pd.read_pickle(test_fname)], axis=0)
        data_dict['test'] = pd.read_pickle(external_fname)
        ...
        if args.task_str == 'classification':
            ...
            if score >= best_auprc_score:
                best_auprc_score = score
                torch.save(model.state_dict(), args.fout_path+'/'+args.prefix_str+'best_model.pt')
        else:
            score = eval_df.loc['RMSE', 'cv_'+str(i)]
            if score <= best_rmse_score:
                best_rmse_score = score
                torch.save(model.state_dict(), args.fout_path+'/'+args.prefix_str+'best_model.pt')
                print(f'current best model=cv{i}: RMSE={score}')
